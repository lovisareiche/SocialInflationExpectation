...
cpi <- read_csv("../SocialInflationExpectation/_intermediate/cpi_cz2000-timeseries_nakamura.csv")
...
cpi_used <- cpi[cpi$date <= max(inflex$date) & cpi$date >= min(inflex$date),]
...
cpi_max3 <- cpi_used[cpi_used$pi_mean == max(cpi_used$pi_mean),]

# what was the average inflation in this time?
mean(cpi_used$pi_mean[cpi_used$date == min(cpi_max3$date)])
...
dat3 <- sci_control %>%
  ...
  # share of connections with cpi experience
  group_by(user_loc) %>%
  summarise(share_high = sum(share_sci[fr_loc %in% cpi_max3$cz2000]), chg_inflex_med, inflexp_median) %>%
  ungroup %>%
  unique
...
# save scatter plot for case 3
png("../SocialInflationExpectation/_output/case_study_3.png", width = 500, height = 500)
plot(dat3$share_high,dat3$inflexp_median, main = "Case Study 3", xlab = "Share of friends in high inflation region", ylab="Median inflation expectations")
dev.off()
